 ...
 *   payload is used for the event specific data that can be used later on (eg, analytics workers populating stat tables, feeds workers populating `feeds` & `feed_actors` etc)
 ...
 *   It allows use to use the payload for other purposes (eg, analytics workers populating stat tables, feeds workers populating feeds from projection tables - `feeds` & `feed_actors` etc)
