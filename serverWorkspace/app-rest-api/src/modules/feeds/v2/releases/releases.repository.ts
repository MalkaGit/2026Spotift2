/**
 * V2 release feed repository: reads from activity_events and activity_event_actors.
  ...
  for (const r of rowList) {
    ...
    if (entry) {
      if (!entry.actors.some((a) => a.id === actorId)) {
        const actorType = entry.eventType === EVENT_TYPE_ALBUM_RELEASED ? ACTOR_TYPE_ARTIST : ACTOR_TYPE_SHOW;
    ...
    const actorType = eventType === EVENT_TYPE_ALBUM_RELEASED ? ACTOR_TYPE_ARTIST : ACTOR_TYPE_SHOW;
