...
import type { AlbumReleasedEventPayload } from "../../../../catalog/activity-events/types/activity-events.payloads";
 ...
 * 2. Map it into rows for `feeds` and `feed_actors`.
 ...
 *    caller's transaction. For example, inserting into `feeds` and
 *    `feed_actors` should usually happen atomically.
 *
 * 
...
export async function insertAlbumReleaseEvent(
  ...
  await insertFeedEvent(conn, {
    ...
    feedType: FEED_TYPE_WHATS_NEW,
    feedVerb: EventTypes.CATALOG_ALBUM_RELEASED,
    feedObjectType: AggregateTypes.ALBUM,
    feedObjectId: payload.album.id,
    feedObjectTitle: payload.album.title,
    feedObjectSubTitle: null,
    feedObjectImageUrl: payload.album.imageUrl ?? null,
