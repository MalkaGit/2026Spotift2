import type { AlbumType } from "../../albums/types/album.model";

/**
 * This file defines the payloads of all the events produced by catalog domain.
 * activity-events infrastructure moduleis responsible for storing these events
 * (right now in single table for all domain events).
 * Each reader domain has worker and offset table that reads the events
 * and writes projections to the projection tables of te reader domain.
 * .
 *
 * Always treat this file as the source of truth for what goes into JSON payloads,
 * even if they are very similar to the ActivityEvent type used to store the canonical envelope.
 * This helps keep the catalog domain decoupled from transport/persistence details.
 */

/**
 * Catalog-owned payload for `catalog.album_released` events.
 *
 * - This is the domain snapshot that gets serialized into `activity_events.event_payload_json`.
 * - Envelope fields (`event_id`, `event_occurred_at`, `event_domain`, `event_type`,
 *   `aggregate_type`, `aggregate_id`, `schema_version`, `created_at`) live on the
 *   `activity_events` row and are not part of this type.
 * - Some values are intentionally duplicated between envelope and payload
 *   (for example `aggregate_id` and `album.id`) so the payload is self-contained
 *   and readable if moved to Kafka, SQS, logs, or another database.
 * - `releasedAt` is typed as `Date`, but when stored in JSON it will be serialized
 *   (typically to an ISO string) and consumers may need to parse it back to a `Date`.
 */
export interface AlbumReleasedEventPayload {
  album: {
    id: string;
    title: string;
    imageUrl: string | null;
    albumType: AlbumType;
  };
  releasedAt: Date;
  artists: {
    id: string;
    name: string;
  }[];
}

/**
 * Catalog-owned payload for `catalog.episode_released` events.
 *
 * - Serialized into `activity_events.event_payload_json`.
 * - Episode belongs to one show; `show` (id, name) is also written to `activity_event_actors` with actor_type = 'show'.
 */
export interface EpisodeReleasedEventPayload {
  episode: {
    id: string;
    title: string;
    imageUrl: string | null;
    durationMs: number;
  };
  releasedAt: Date;
  show: {
    id: string;
    name: string;
  };
}
